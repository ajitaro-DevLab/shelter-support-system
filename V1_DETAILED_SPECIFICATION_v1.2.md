...
本書は、V1実証版の詳細仕様・制約・テスト条件を定義する、最上位の実装仕様書である。
...
- confidence の表示判定は、Home・HQ・その他の参照画面で個別実装せず、共通serviceまたは共有関数を1箇所に実装して使用する。特に「現在確認枠が未実施なら表示上 unconfirmed」の判定は全APIで同一ロジックを使用すること。
...
表示値
confirmed / estimated / unconfirmed
