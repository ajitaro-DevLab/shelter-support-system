...
- 情報鮮度（最終正式確認時刻からの経過時間を基準に4段階で表示）と信頼度（確定／推定／未確認）の表示
...
- 実際の災害現場や避難所運営訓練での検証は未実施です。同一LAN内でのスマートフォン実機・複数ブラウザによる共有確認は実施済みです。Android版Chromeでの動作を確認済みですが、Android版Firefoxでは複数実機でソフトウェアキーボードから文字・数値を直接入力できない事象を確認しています（Chrome for Androidでは発生せず、原因は断定していません）。V1実証版ではAndroid端末の推奨ブラウザーをChromeとしています。
- 1避難所につき主入力端末1台を基本運用とする前提です。同一避難所への複数端末からの同時更新に対する高度な競合解決は実装していません（複数端末からの参照は可能です）。
...
- [`V1_DETAILED_SPECIFICATION_v1.2.md`](V1_DETAILED_SPECIFICATION_v1.2.md)：V1実証版の詳細仕様（最上位の仕様書。DB・API・業務ロジックの詳細はこちら）
- [V1.0通し試験シナリオ](V1_TEST_SCENARIO.md)：ブラウザ操作による9イベントの手動通し試験手順（対応する自動テストは`backend/integration-tests/v1-day-scenario.test.js`）
